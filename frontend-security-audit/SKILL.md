...
**跳过：** dist/ build/ node_modules/ .next/ .nuxt/ .svelte-kit/ coverage/ vendor/ generated/ report/ .audit/ .bsaf/ __tests__/ __mocks__/ cypress/ e2e/ *.test.* *.spec.* *.stories.* *.min.js *.d.ts *.map .env.example .env.sample .env.template `security-audit-report*.md`；单文件 >2MB 跳过。

**输出：** 唯一正式产出为对话中的最终确认报告；无漏洞时输出结论并含局限性声明。默认**不向被审计项目写入** `security-audit-report-YYYYMMDD.md`、`.audit/` 或其他中间文件。
