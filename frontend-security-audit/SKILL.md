...
> **仅最后输出报告**：Phase 0/1/2 及 Cross-Shard 的 recon、todo-list、判定卡等**中间内容均不向用户展示**；用户只会看到一份最终确认报告（默认直接输出到对话）。
...
**只读约束：** 本 Skill 只负责发现和确认前端真实可利用的安全问题，**不得修改**被审计项目中的源码、配置、依赖、测试、文档、脚本或构建产物；即使用户要求 `include_fix=true`，也只输出修复建议，不直接改仓库。

**证据约束：** `report/`、`.audit/`、`security-audit-report*.md` 视为**派生审计产物**；可作为 finding 线索来源，但**绝不能**作为代码实现证据或漏洞成立/不成立的依据。`.bsaf/` 若存在，也仅可作为定位和补线索输入，不能直接定案。审计时默认跳过这些产物。
...
| phase | ❌ | 手动指定0/1/2 |
| context | ❌ | 补充上下文 |
| persist_todo | ❌ | false→关闭 TODO 落盘，仅保留对话上下文；默认 true |

**跳过：** dist/ build/ node_modules/ .next/ .nuxt/ .svelte-kit/ coverage/ vendor/ generated/ report/ .audit/ .bsaf/ __tests__/ __mocks__/ cypress/ e2e/ *.test.* *.spec.* *.stories.* *.min.js *.d.ts *.map `security-audit-report*.md`；单文件 >2MB 跳过。`.env.example`/`.env.sample`/`.env.template` **不跳过**，纳入凭证扫描（checks-credentials.md）。

**输出：** 唯一正式产出为对话中的最终确认报告；无漏洞时输出结论并含局限性声明。默认**不向被审计项目写入** `security-audit-report-YYYYMMDD.md`、`.audit/` 或其他中间文件；唯一例外是 `.bsaf/cevf-todo.md`（TODO 持久化，可通过 `persist_todo=false` 关闭）。

**对用户的输出约定（必须遵守）：**
- **整个审计流程中，仅在全部 Phase 完成后向用户输出一份最终报告**。
- **中间阶段不向用户输出任何内容**：recon-profile、分片计划、todo-list、判定卡、Phase 完成提示、Shard 进度等**均不输出到对话**，仅在内部保持；用户可见的只有最后的确认报告。
...
**状态持久化（中间不输出到对话）：**
```
中间产物（recon-profile、todo-list、判定结果等）不输出到对话，仅在内部/上下文中保持，不向用户展示；最终仅输出报告时再汇总
```
用户在整个流程中只会在最后看到一份报告。

**TODO 持久化（默认开启）：** Phase 1 每个 Shard 完成后，自动将本 Shard 的 todo-list append 写入 `.bsaf/cevf-todo.md`（文件不存在则创建）；Phase 2 开始时从该文件加载完整待验列表，每条验证完毕后立即更新 status 写回文件。可通过参数 `persist_todo=false` 关闭，改为仅保留对话上下文。`.bsaf/cevf-todo.md` 是本 Skill 唯一默认写盘文件，不含报告内容。

**自驱动协议（自动续接 + 仅最后输出报告）：**
1. 完成当前 Phase/Shard 后**自动进入下一 Phase 或下一 Shard**，**不向用户输出**任何中间产物或进度提示。
2. 中间阶段的结构化产物（recon-profile、todo-list、判定卡）**仅在内部保持**，不在对话中展示；同一回复内或下一轮直接执行 Phase Router 决定的下一步，直至全部 Phase 完成。
3. **仅当 Phase 2 全部验证完成时**，生成最终报告并**作为唯一内容输出给用户**。
4. **仅以下情况向用户输出非报告内容**：跨分片对账时无法从上下文稳定汇总 todo-list 时，可简短提示用户「审计进行中，请稍候」或静默继续；用户主动说「暂停」「先到这里」时回复确认。
...
P2-P5仅摘要行。超限→P0→P5截断，余项记入「未扫描Sink清单」。**不输出到对话**；将 todo-list 在内部保持后，若有未扫描分片则进入下一 Shard，否则进入 Cross-Shard 对账，再进入 Phase 2。
...
若无法汇总（如早期 Shard 已滚出上下文）→ 在内部尽可能合并已有信息后继续，跨 Shard 链路标⚪待确认；仍不输出中间内容。
...
报告格式按`references/rule-index.md` §report-template 输出 → 默认完整报告输出到对话；仅在用户明确要求且目标为**仓库外路径**时，才可导出文件。

**→ 此时才向用户输出**：将上述报告作为**唯一**展示给用户的内容。不输出「Shard X 验证完成」等中间提示；全部完成后仅输出最终报告。
