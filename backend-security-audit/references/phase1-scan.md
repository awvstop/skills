...
## Shard TODO 落盘（Reflection 完成后立即执行）

将本 Shard 所有 TODO 条目 append 写入 `.bsaf/bsaf-todo.md`（文件不存在则创建）。字段与 core.md TODO 定义对齐，每行格式：

```
{id} | {path} | {severity} | {confidence} | {file→function} | {sink_type} | {source_type} | {trust} | {evidence_source} | {status}
```
...
- path：`A`/`B`/`C`（正向/反向/结构）
- severity：`P0`–`P5`
- confidence：`Confirmed`/`Likely`/`Candidate`/`Boundary`
- file→function：`src/api/user.js→getUser`
- sink_type：`sql`/`ssrf`/`cmd` 等（见 core.md Sink 类型）
- source_type：原始输入表达式，如 `req.params.id`
- trust：`🔴`/`🟠`/`🟡`/`🟢`/`⚪`
- evidence_source：`grep_output`/`symbol_reference`/`file_read` 等
- status：`pending`

示例：`S1-003 | B | P0 | Confirmed | src/api/user.js→getUser | sql | req.params.id | 🔴 | grep_output,file_read | pending`

写入完成后输出一行确认：「Shard N TODO 已落盘（X 条，累计 Y 条）」，再自动推进。

**自动推进：**
- todo-list 还有未扫描分片 → 自动开始下一 Shard（无需用户确认）
- 全部分片完成 → 自动加载 `references/cross-shard.md` 执行跨分片分析
- Cross-Shard 完成 → 自动加载 `references/phase2-verify.md` 进入 Phase 2
