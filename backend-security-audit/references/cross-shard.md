...
1. 读取 `.bsaf/bsaf-todo.md` 获取所有 Shard 的 TODO 条目（比对对话上下文，以文件为准）
2. 识别跨 Shard 数据流
3. [shared] 函数影响范围 → 复制到受影响 Shard
4. 服务间调用边界标记
5. 组合升级（见下）
6. 更新 todo-list，将新增条目（跨 Shard 组合、[stored-source] 展开）append 写入 `.bsaf/bsaf-todo.md`；新增条目 id 格式为 `CS-{seq}`（如 `CS-001`），与 Shard 域条目区分
...
shard_stub:
  shard_id: N
  todos:
    - "S1-001 | B | P0 | Confirmed | src/api/user.js→getUser | sql | req.params.id | 🔴 | grep_output,file_read | pending"
  open_sources: []
  boundary_calls: []
```

> shard_stub 格式与 `.bsaf/bsaf-todo.md` 一致（见 phase1-scan.md TODO 落盘格式）；优先从文件读取，文件不可用时才请用户粘贴此格式。
