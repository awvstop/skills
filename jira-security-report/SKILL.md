...
- **写入失败**：仅输出报告正文（8 部分 + CVSS 最后两行），前后无前言、无总结句。
- **多份报告**：仅一句列出路径，如「已写入 report/ 下 N 份报告：xxx.md, yyy.md」。
- **报告正文**：仅 **8 部分 + CVSS 最后两行**（结构见下），不增删章节；**中文**；专业、简洁、工程化、面向开发可执行；**仅描述现象与影响**，不断言「设计本意」；**不编造**描述、路径、代码片段与影响结论。
- **CVSS**：须为整份报告的**最后两行**（严重程度 + 链接），格式见本文档末尾；逐项分析 AV/AC/PR/UI/S/C/I/A，从攻击者视角取结合实际可利用性，造成的实际危害取最贴合实际的值。（AV=N/L/A/P；**AC=L/H**；PR=N/L/H；UI=N/R；S=U/C；C/I/A=N/L/H）

---

## 报告结构（仅输出以下 8 部分 + CVSS）
...
- **格式**：每句话单独成段（句号后换行），不写成连续长段；以可落地的修复动作为主，不展开设计意图讨论。

---

### 验收标准（Acceptance Criteria）

- **内容**：1～3 条可操作的验证动作，供开发自测或安全复验使用；描述**预期结果**而非修复动作（如「重放 PoC 请求，应返回 403」「参数 X 传入特殊字符，应被拒绝并记录日志」）。
- **格式**：编号列表（1. 2. 3.）；每条一句，简洁可执行；**不编造**无法从漏洞现象推导的验证步骤。
...
### CVSS 3.1（必须是整份文档的最后两行）

> **生成指令（不写入报告正文）**：根据 CVSS Base Score 映射严重程度（9.0-10.0 = Critical；7.0-8.9 = High；4.0-6.9 = Medium；0.1-3.9 = Low）；将下方 `*` 逐项替换为实际取值；AV=N/L/A/P；AC=L/H；PR=N/L/H；UI=N/R；S=U/C；C/I/A=N/L/H。从攻击者视角，取结合实际可利用性与危害最贴合的值。

**严重程度**：Critical / High / Medium / Low（按上方映射填一个）
...
- `验收标准` 是否为编号列表，且每条描述的是预期结果而非修复动作。
- `严重程度` 是否已从 CVSS Base Score 映射填写，且与 CVSS 链接相邻为最后两行。
